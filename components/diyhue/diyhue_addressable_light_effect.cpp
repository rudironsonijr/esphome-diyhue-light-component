...
#include "diyhue.h"
#include "diyhue_addressable_light_effect.h"
...
namespace esphome
{
  namespace diyhue
  {
    ...
    static const char *const TAG = "diyhue_addressable_light_effect";

    DiyHueAddressableLightEffect::DiyHueAddressableLightEffect(const std::string &name) : AddressableLightEffect(name) {}

    const std::string &DiyHueAddressableLightEffect::get_name() { return AddressableLightEffect::get_name(); }

    void DiyHueAddressableLightEffect::start()
    {
    ...
    void DiyHueAddressableLightEffect::stop()
    {
      ...
      DiyHueLightEffectBase::stop();
      AddressableLightEffect::stop();
    }

    void DiyHueAddressableLightEffect::apply(light::AddressableLight &it, const Color &current_color)
    {

      // if receiving DiyHue packets times out, reset to home assistant color.
    ...
    uint16_t DiyHueAddressableLightEffect::process_(const uint8_t *payload, uint16_t size, uint16_t used)
    {
      ...
      // effect is active once a diyhue packet is received.
      ...
      ESP_LOGV(TAG, "Applying DiyHue data for '%s' (size: %d - used: %d - num_pixels: %d)", get_name().c_str(), size, used, num_pixels);
      ...
      {
      case DIYHUE_SCALE_PACKET:
        multiplier = this->scan_packet_and_return_multiplier_(payload, 10, size);
        set_max_brightness_();
        break;
      case DIYHUE_SCALE_STRIP:
        multiplier = this->scan_packet_and_return_multiplier_(payload, used, used + (num_pixels * 3));
        set_max_brightness_();
        break;
      case DIYHUE_NO_SCALING:  // no scaling requires brightness maxed so that diyhue values will be displayed raw.
      case DIYHUE_SCALE_PIXEL: // pixel scaling occurs at the pixel level, no need to scan here but we still need brightness maxed.
        set_max_brightness_();
      default:
      ...
      for (uint16_t i = used; i < used + (num_pixels * 3); i += 3)
      {
        ...
        if (this->scaling_mode_ == DIYHUE_SCALE_PIXEL)
        {
          uint8_t max_val = 0;
    ...
    float DiyHueAddressableLightEffect::scan_packet_and_return_multiplier_(const uint8_t *payload, uint16_t start, uint16_t end)
    {
      ...
      // diyhue values are shown raw.
      // if all pixels are off, brightness doesn't matter.
      return multiplier_from_max_val_(max_val);
    }

    float DiyHueAddressableLightEffect::multiplier_from_max_val_(uint8_t max_val)
    {
      if (max_val == 0)
      {
        return 1.0f;
    ...
    void DiyHueAddressableLightEffect::set_max_brightness_()
    {
