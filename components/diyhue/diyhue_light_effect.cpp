...
namespace esphome
{
  namespace diyhue
  {
    ...
    static const char *const TAG = "diyhue_light_effect";

    DiyHueLightEffect::DiyHueLightEffect(const std::string &name) : LightEffect(name) {}

    const std::string &DiyHueLightEffect::get_name() { return LightEffect::get_name(); }

    void DiyHueLightEffect::start()
    {
    ...
    uint16_t DiyHueLightEffect::process_(const uint8_t *payload, uint16_t size, uint16_t used)
    {
      ...
      // for DIYHUE_PACKET mode, find largest r,g, or b value in packet and scale that value to brightness
      if (this->scaling_mode_ == DIYHUE_SCALE_PACKET)
      {
      ...
      // DIYHUE_PIXEL and DIYHUE_STRIP are the same with bulbs since a "strip" is one pixel.
      if ((this->scaling_mode_ == DIYHUE_SCALE_STRIP) || (this->scaling_mode_ == DIYHUE_SCALE_PIXEL))
      {
      ...
      if (this->scaling_mode_ != DIYHUE_NO_SCALING)
      {
        red *= multiplier;
        green *= multiplier;
        blue *= multiplier;
      ...
      // manually calling loop otherwise we just go straight into processing the next DiyHue
